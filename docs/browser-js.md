...
- **Web API stubs** (`webapi_stubs.c`): `fetch` is GET-only with same-origin/CORS gating (not a full `Response`); non-GET / `signal` / `body` init fail closed. `localStorage`/`sessionStorage` are in-memory per-tab maps (not disk); quota exhaustion fails closed. `AbortController` is not installed (unsupported — no fake shell). DOM bridge stays in `browser_js.c`.
