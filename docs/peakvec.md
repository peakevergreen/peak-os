...
- **Brute-force cosine** over int16 vectors (`PEAKVEC_DIM=64`). The query path caches corpus norms, normalizes the query once, uses batched dot/norm helpers, and early-outs scores that cannot enter top-k. ANN (HNSW/IVF) is still deferred until corpora need it — revisit when a namespace regularly exceeds ~512 live entries or query latency shows up in Monitor/sysmon.
