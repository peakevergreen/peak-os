...
void shell_help_topics(void) {
    ...
    console_write("  text  cat head tail wc grep diff sort uniq cut tr sed cmp hexdump strings echo printf tee yes edit\n");
    console_write("  sys   date free top sysmon ps kill env which seq sleep theme wallpaper scale\n");
    console_write("        uname true false test [ yes sh reboot help man js\n");
