/*
 * crypto_p256.c — NIST P-256 ECDH for TLS ECDHE.
 * Adapted from p256-m by Manuel Pégourié-Gonnard (Apache-2.0) for Peak OS.
 * SPDX-License-Identifier: Apache-2.0
 */
#include "crypto.h"
#include "util.h"

#define MUL64_IS_CONSTANT_TIME 1
...
static int p256_generate_random(uint8_t *output, unsigned output_size) {
    if (crypto_random(output, output_size) != 0)
        return -1;
    return 0;
}
 ...
 */
#if !defined(PEAK_HOST_TEST) && !defined(__aarch64__) && !defined(__arm64__) && \
    defined(__GNUC__) && defined(__ARM_ARCH) && __ARM_ARCH >= 6 && \
    defined(__ARM_ARCH_PROFILE) && \
...
int p256_keygen(uint8_t priv[32], uint8_t pub_uncompressed[65]) {
    uint8_t pub[64];
    if (p256_gen_keypair(priv, pub) != 0)
        return -1;
    pub_uncompressed[0] = 0x04;
    memcpy(pub_uncompressed + 1, pub, 64);
    return 0;
}

int p256_ecdh(uint8_t shared_x[32], const uint8_t priv[32],
              const uint8_t peer_uncompressed[65]) {
    if (!peer_uncompressed || peer_uncompressed[0] != 0x04)
        return -1;
    return p256_ecdh_shared_secret(shared_x, priv, peer_uncompressed + 1);
}
