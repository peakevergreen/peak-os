...
#include "rsa_ske_vectors.h"
    ...
    /* HKDF-SHA256 RFC 5869 Test Case 1 (basic). */
    {
        ...
        static const uint8_t info[] = {0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6,
                                       0xf7, 0xf8, 0xf9};
        static const uint8_t okm_exp[] = {
        ...
        hkdf_expand_sha256(prk, info, sizeof(info), okm, sizeof(okm));
        expect(!memcmp(okm, okm_exp, sizeof(okm_exp)), "hkdf sha256 rfc5869 case1");
        uint8_t label_out[16];
        expect(tls13_hkdf_expand_label(0, prk, 32, "key", NULL, 0, label_out, 16) == 0,
               "tls13 expand_label");
        int nz = 0;
        for (int i = 0; i < 16; i++)
            nz |= label_out[i];
        expect(nz, "tls13 expand_label nonzero");
    }
}
